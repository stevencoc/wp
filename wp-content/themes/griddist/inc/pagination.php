...
if ( $query_args['max_num_pages'] > $query_args['paged'] ) :
	...
	<section class="pagination-wrapper mpad-u-0 mpad-d-80 tpad-d-100 dpad-d-180">
		...
		<div id="pagination" data-query-args="<?php echo esc_attr( $json_query_args ); ?>" data-load-more-target=".load-more-target">
			...
			<button id="load-more" class="mfs-32 tfs-36 dfs-48 color-dark-gray color-black-hover"><?php _e( 'Load more', 'griddist' ); ?></button>

			<p class="out-of-posts"><?php _e( 'Nothing more to load.', 'griddist' ); ?></p>

			<div class="loading-icon">
				<?php griddist_loading_indicator(); ?>
			</div>
			...
			if ( $has_previous_link || $has_next_link ) :
				...
				<nav class="link-pagination<?php echo $pagination_class; ?>">
					...
					<?php if ( get_previous_posts_link() ) : ?>
						<?php previous_posts_link( __( '&larr; Previous Page', 'griddist' ) ); ?>
					<?php endif; ?>

					<?php if ( get_next_posts_link() ) : ?>
						<?php next_posts_link( __( 'Next Page &rarr;', 'griddist' ) ); ?>
					<?php endif; ?>
